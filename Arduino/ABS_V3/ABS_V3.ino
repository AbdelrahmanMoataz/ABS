...
#define CLOSED 180
#define LEFT_OPEN 85
#define LEFT_CLOSED 95
#define RIGHT_OPEN 85
#define RIGHT_CLOSED 75
...
double BRAKING_PERIOD = 5000/255; // 5000 ms divided by 255 units will make sure speed is reduced gradually to zero by 5 seconds
...
void calculateSpeed(int wheel_sensed, double* wheel_velocity){
    ...
    if(end_time[index] - start_time[index] > PERIOD){
        ...
        *wheel_velocity = (diff * PI * WHEEL_DIAMETER / (NUMBER_OF_SLITS * 2)) / (PERIOD); // multiplying number of slits by 2 because steps are read by change of state instead of at HIGH
        //*wheel_velocity = (diff * 1.0 / (NUMBER_OF_SLITS *2)) / (PERIOD / 1000);
...
void carMode(String mode) {
    ...
    if(mode == "S"){
        ...
        Serial.println("Start moving at max speed!");
    }

    // Stop: The car instantly stops, motor acts as brakes
    if(mode == "s"){
      carControl(STOP, 0);
      Serial.println("Stopped the car!");
    }
    ...
    if(mode == "A"){
        ...
        Serial.println("Switched to ABS mode!");
    }

    // Brakes: Starts the braking process
    if(mode == "B"){
        // Car starts moving at max speed for 0.5 seconds
        Serial.println("Started the braking process!");
        int speed = 255;
        carControl(FORWARD,speed);
        delay(500);
        ...
        while (speed >= 0) {
            ...
            current = millis();
            if (current - last > BRAKING_PERIOD) { // this if condition ensures motors stop rotating after 5 seconds
        ...
        Serial.println("End of braking process!");
    }
}
